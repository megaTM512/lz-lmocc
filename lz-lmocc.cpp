...
// Double Binary Search to first find the range of occurrences, then find the
// earliest occurrence
int64_t getEarliestOccurrence(const PhraseC& phrase, const std::string& text,
  ...
  for (int64_t i = L + 1; i <= R; ++i) {
    best = std::min(best, SA[i]);
  }
  return best;
}

std::vector<PhraseC> lmocc(std::vector<PhraseC>& phrases) {
  // Phrase Decoding
  ...
  // Find earliest occurrences for every phrase & update positions
  for (auto& phrase : phrases) {
    int64_t newEarlyPos = getEarliestOccurrence(phrase, text, SA);
    if (phrase.pos != newEarlyPos && newEarlyPos != -1) {
...
int main(int argc, char* argv[]) {
  ...
  std::cout << "LZ-LM-OCC Compression Enhancement Tool" << std::endl;
  ...
  // LZ-LM-OCC Processing
  auto start = std::chrono::high_resolution_clock::now();
  auto outputPhrases = lmocc(inputPhrases);
  auto end = std::chrono::high_resolution_clock::now();
  std::chrono::duration<double> elapsed = end - start;
  std::cout << "LZ-LMOCC processing time: " << elapsed.count() << " seconds."
            << std::endl;
